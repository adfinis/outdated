...
if TYPE_CHECKING:
    ...
    from outdated.outdated.factories import (
        DependencyFactory,
        ReleaseVersionFactory,
        VersionFactory,
    )
...
@pytest.mark.django_db()
def test_fetch_end_of_life_no_overwrite_if_already_set(
    mocker: MockerFixture,
    ...
    mocker.patch.object(
        LockfileParser, "_get_end_of_life_date", return_value=date(2026, 2, 2)
    )

    dependency: models.Dependency = dependency_factory(
        name="ember-data", provider="NPM"
    )

    release_version: models.ReleaseVersion = release_version_factory(
        major_version=4, minor_version=4, end_of_life=end_of_life, dependency=dependency
    )
    ...
    assert release_version.end_of_life == end_of_life
